...
const AnimeList = ({api}) => {
    return (
        ...
        {api.data?.map((anime, index) => {
            return (
                <>
                    <div className="border border-color-primary rounded">
                        <Link href={`/anime/${anime.mal_id}`} className="cursor-pointer text-color-primary hover:text-color-accent transition-all" key={index}>
                            <Image src={anime.images.webp.image_url} alt="..." width={350} height={350} className=" w-full max-h-64 md:object-cover"/>
